...
def download_station_file(station_id: str, start = None, end = None) -> pd.DataFrame:
    ...
    if start is not None and end is not None:
        ...
        df['__east(m)'] = df['__east(m)'] - list(df['__east(m)'])[0]
        df['_north(m)'] = df['_north(m)'] - list(df['_north(m)'])[0]
        df['____up(m)'] = df['____up(m)'] - list(df['____up(m)'])[0]
...
def find_stations(timeseries: str, coherence: str, geometry: str) -> tuple:
    ...
    with h5py.File(timeseries, 'r') as f:
        ts_start = datetime.strptime(f.attrs['START_DATE'], '%Y%m%d')
        ts_end = datetime.strptime(f.attrs['END_DATE'], '%Y%m%d')
    ...
    with h5py.File(coherence, 'r') as f:
        ...
        for i in range(len(list(df["Long(deg)"]))):
            x, y, z, n = utm.from_latlon(list(df["Lat(deg)"])[i], list(df["Long(deg)"])[i], force_zone_number=z, force_zone_letter=n)
...
def find_reference_station(coherence: str, stations: list, lons_gps: list, lats_gps: list):
    """Reference time series file with respect to the farthest GPS station.

    Args:
        cohrence: cohrence file.
        stations: Names of the stations.
        lons_gps: Longitude coordinates of the stations.
        lats_gps: Latitude coordinates of the stations.
...
def plot_comparison(timeseries: str, coherence: str, geometry: str) -> None:
    ...
    with h5py.File(timeseries, 'r') as f:
        ...
        ts_end = datetime.strptime(f.attrs['END_DATE'], '%Y%m%d')
        dates = f['date'][:]
        dates=[datetime.strptime(date.decode('utf-8'),'%Y%m%d') for date in dates]
    ...
    for i, sta in enumerate(stations):
        ...
        df_sta['__east(m)'] = df_sta['__east(m)'] - list(df_sta['__east(m)'])[0]
        df_sta['_north(m)'] = df_sta['_north(m)'] - list(df_sta['_north(m)'])[0]
        df_sta['____up(m)'] = df_sta['____up(m)'] - list(df_sta['____up(m)'])[0]
        ts_ref = f"{timeseries.split('.h5')[0]}_ref.h5"

        los_gps=-(df_sta['__east(m)']*np.sin(np.radians(inc_gps[i]))*np.cos(np.radians(90-az_gps[i]))-df_sta['_north(m)']*np.sin(np.radians(inc_gps[i]))*np.sin(np.radians(90-az_gps[i]))-df_sta['____up(m)']*np.cos(np.radians(inc_gps[i])))
